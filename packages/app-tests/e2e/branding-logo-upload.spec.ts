...
const enableBrandingAndUpload = async (page: Page) => {
  ...
  // Two logo tiles now exist (light and dark); target the light one explicitly.
  await page.getByTestId('branding-logo').locator('input[type="file"]').setInputFiles(LOGO_PATH);
...
test('[BRANDING_LOGO]: clears the organisation branding logo when the user removes it', async ({ page }) => {
  ...
  await page.getByTestId('branding-logo').getByRole('button', { name: 'Remove' }).click();
  await page.getByRole('button', { name: 'Save changes' }).first().click();
